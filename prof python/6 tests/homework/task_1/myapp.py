...
def check_document_existance(user_doc_number: str) -> bool:
    doc_founded = False
...
def show_document_info(document):
    ...
    res = ('{} "{}" "{}"'.format(doc_type, doc_number, doc_owner_name))
    return res
...
def show_all_docs_info():
    ...
    for current_document in documents:
        info = show_document_info(current_document)
        print(info)
    return True
